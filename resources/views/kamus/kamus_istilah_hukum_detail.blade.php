				...
				<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 mb30" data-mh="equal-block">					
					<div class="row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">		
							<p class="link text-dark mb-2 mt-2">Hasil Pencarian <strong><span class="sen">'@foreach($keys as $k) {{trim($k->indo) ?? trim($k->asing)}} @endforeach'</span></strong></p>
							<hr class="mb-3">
							
							<h1 class="cap mb-3 mt-3 uppercase h5">Pengertian @foreach($keys as $k) {{$k->indo ?? $k->asing}} @endforeach</h1>
							<p><strong>
								<span class="cap">@foreach($keys as $k) {{$k->indo ?? $k->asing}} @endforeach</span>
								</strong> memiliki {{$kamus->count() ?? 1}} arti. <strong>
								<span class="cap">@foreach($keys as $k) {{$k->indo ?? $k->asing}} @endforeach</span>
